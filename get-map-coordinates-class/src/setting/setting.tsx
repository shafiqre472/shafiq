...
> {
  ...
  onShowTiltPropertyChange = (evt: React.FormEvent<HTMLInputElement>) => {
    this.props.onSettingChange({
      id: this.props.id,
      config: this.props.config.set("showTilt", evt.currentTarget.checked)
    });
  };

  onShowRotationPropertyChange = (evt: React.FormEvent<HTMLInputElement>) => {
    this.props.onSettingChange({
      id: this.props.id,
      config: this.props.config.set("showRotation", evt.currentTarget.checked)
    });
  };
      ...
      <div css={style}>
        <div className="widget-setting-get-map-coordinates">
          ...
          >
            ...
            <SettingRow>
              <div className="w-100 showTilt">
                <div className="checkbox-row">
                  <label>
                    <FormattedMessage
                      id="showTilt"
                      defaultMessage={defaultI18nMessages.showTilt}
                    />
                  </label>
                  <Switch
                    checked={
                      (this.props.config && this.props.config.showTilt) ||
                      false
                    }
                    onChange={this.onShowTiltPropertyChange}
                  />
                </div>
              </div>
            </SettingRow>
              ...
              <div className="w-100 showTilt">
                <div className="checkbox-row">
                  <label>
                    <FormattedMessage
                      id="showRotation"
                      defaultMessage={defaultI18nMessages.showRotation}
                    />
                  </label>
                  <Switch
                    checked={
                      (this.props.config && this.props.config.showRotation) ||
                      false
                    }
                    onChange={this.onShowRotationPropertyChange}
                  />
                </div>
              </div>
            </SettingRow>
